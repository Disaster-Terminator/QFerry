...
- fixture provider 可调用 `get_status`、`list_mailboxes`、`get_mailbox_summary`、`search`、`classify_messages`、`triage_inbox`、`group_spam_candidates`、`classification_sweep`、`classification_map`、`plan_cleanup`。
- QQ Mail provider 可调用 `get_status`、`get_capability_snapshot`、`list_mailboxes`、`get_mailbox_summary`、bounded `search`、按 UID `fetch`、`triage_inbox`、`classification_sweep`、`classification_map` 和 `group_spam_candidates`。
...
| 分类地图 | `classification_map` 对选中窗口按安全/购买/营销/newsletter/开发社区/待审分桶，并给出建议动作，不生成操作计划 |
