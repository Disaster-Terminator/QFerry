...
`confirmCleanupPlan` and `executeCleanup` are intentionally separate. Confirmation records that the user approved one specific preview plan saved by the MCP server; execution consumes that confirmed `operationPlanId` exactly once. Clients must not send a fabricated `status: "confirmed"` plan object.

Operation plans are durable MCP state, not provider state and not process-local memory. Capability flags such as `mutationAllowed`, `mutationOperationallyReady`, and `supportsNativeMove` describe whether the provider can perform the requested mutation, but they do not prove that an earlier preview plan can be recovered. The default MCP server persists plans under the QFerry state directory, or under `QFERRY_OPERATION_PLAN_STORE_DIR` when configured. That is sufficient for local restarts and single-instance remote MCP deployments. Horizontally scaled cloud deployments must point all instances at the same durable store, or replace the file store with an equivalent KV/DB-backed adapter.
