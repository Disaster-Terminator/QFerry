...
const DEFAULT_PLUGIN_DIR = path.resolve("plugins/qferry");
...
async function resolveCacheRoots(options) {
  ...
  if (options.includeWindows) {
    ...
    if (windowsRoot) roots.push(windowsRoot);
  }
...
async function detectWindowsCacheRoot() {
  if (process.platform === "win32") return defaultCurrentCacheRoot();
...
async function detectWslCacheRoot() {
  ...
  try {
    const { stdout } = await execFileAsync(
      "wsl.exe",
      ["-e", "bash", "-lc", "wslpath -w ~/.codex/plugins/cache"],
...
async function findInstalledPluginTargets(cacheRoot, filters) {
  ...
  for (const marketplace of await readDirNames(cacheRoot)) {
    if (filters.marketplaceName && marketplace !== filters.marketplaceName) continue;
...
async function replaceDirectory(sourceDir, targetDir) {
  ...
  const tempDir = path.join(parent, `.qferry-sync-${process.pid}-${Date.now()}`);
